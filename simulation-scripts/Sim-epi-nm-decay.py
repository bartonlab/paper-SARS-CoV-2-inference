...
def main():
    ...
    errors_truedecay, inferred_truedecay = [], []
    actual, mutant_sites = [], []
    traj = []
    pop_size_normal = []
    errors_falsedecay, inferred_falsedecay = [], []
    ...
    for i in range(n_runs):
        ...
        """
        inference.main(["--data", out1+".npz","--g1",str(g1),"-o",out2+'-falsedecay',"-R", str(2), "--record", str(record), "--freq_cutoff", str(freq_cutoff), "--pop_size", str(int(pop_limit)), '-k', str(k), '--decay_rate', str(decay_rate*2), '--nm_popsize', str(pop_size)])
        inf_data = np.load(out2 + '-falsedecay.npz', allow_pickle="TRUE")
        errors_falsedecay.append(np.array([i for i in inf_data['error_bars']]))
        inferred_falsedecay.append(np.array([i for i in inf_data['selection']]))
        
        inference.main(["--data", out1+".npz","--g1",str(g1),"-o",out2+'-truedecay',"-R", str(2), "--record", str(record), "--freq_cutoff", str(freq_cutoff), "--pop_size", str(int(pop_limit)), '-k', str(k), '--decay_rate', str(decay_rate), '--nm_popsize', str(pop_size)])
        inf_data_nm = np.load(out2 + '-truedecay.npz', allow_pickle="TRUE")
        errors_truedecay.append(np.array([i for i in inf_data_nm['error_bars']]))
        inferred_truedecay.append(np.array([i for i in inf_data_nm['selection']]))
        
    
    # save the MSE array, trajectory array, and averaged selection coefficients array
    f = open(out_str+'.npz', mode='w+b')
    np.savez_compressed(f, traj=traj, inferred_truedecay=inferred_truedecay, errors_truedecay=errors_truedecay, inferred_falsedecay=inferred_falsedecay, errors_falsedecay=errors_falsedecay,
