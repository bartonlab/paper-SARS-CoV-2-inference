...
def main():
    ...
    parser.add_argument('--decay_rate', type=float,      default=1,                       help='the decay rate at which individuals become no longer infectious')
    ...
    errors_truepop, inferred_truepop = [], []
    actual, mutant_sites = [], []
    traj = []
    pop_size_normal = []
    errors_falsepop, inferred_falsepop = [], []
    ...
    for i in range(n_runs):
        ...
        wrong_pop      = np.ones(len(data['pop_size'][0])) * 10000
        wrong_pop_file = out1 + 'wrong-pop.npy'
        np.save(wrong_pop_file, wrong_pop)
        ...
        """
        inference.main(["--data", out1+".npz","--g1",str(g1),"-o",out2+'-falsepop',"-R", str(2), "--record", str(record), "--freq_cutoff", str(freq_cutoff), "--pop_size", str(int(pop_limit)), '-k', str(k), '--decay_rate', str(decay_rate), '--nm_popsize', str(wrong_pop_file)])
        inf_data = np.load(out2 + '-falsepop.npz', allow_pickle="TRUE")
        errors_falsepop.append(np.array([i for i in inf_data['error_bars']]))
        inferred_falsepop.append(np.array([i for i in inf_data['selection']]))
        
        inference.main(["--data", out1+".npz","--g1",str(g1),"-o",out2+'-truepop',"-R", str(2), "--record", str(record), "--freq_cutoff", str(freq_cutoff), "--pop_size", str(int(pop_limit)), '-k', str(k), '--decay_rate', str(decay_rate), '--nm_popsize', str(pop_size)])
        inf_data_nm = np.load(out2 + '-truepop.npz', allow_pickle="TRUE")
        errors_truepop.append(np.array([i for i in inf_data_nm['error_bars']]))
        inferred_truepop.append(np.array([i for i in inf_data_nm['selection']]))
        
    
    # save the MSE array, trajectory array, and averaged selection coefficients array
    f = open(out_str+'.npz', mode='w+b')
    np.savez_compressed(f, traj=traj, inferred_truepop=inferred_truepop, errors_truepop=errors_truepop, inferred_falsepop=inferred_falsepop, errors_falsepop=errors_falsepop,
