...
def main():
    ...
    spec = importlib.util.spec_from_file_location("epi", "/rhome/blee098/epi.py")
    ...
    spec3  = importlib.util.spec_from_file_location("epi-nonmarkovian", "/rhome/blee098/epi-nonmarkovian.py")
    epi_nm = importlib.util.module_from_spec(spec3)
    spec3.loader.exec_module(epi_nm)
    
    # initialize arrays that will be used later
    errors, inferred, traj          = [], [], []
    actual, mutant_sites            = [], []
    pop_size_normal, pop_size_nm    = [], []
    errors_nm, inferred_nm, traj_nm = [], [], []
    ...
    for i in range(n_runs):
        ...
        epi.main(["-o", out1, "--timed", str(timed),"-N",str(N),"--mu",str(mu),"--sample", str(sample), "--record", str(record),
        ...
        epi_nm.main(["-o", out1 + '-nm', "--timed", str(timed),"-N",str(N),"--mu",str(mu),"--sample", str(sample), "--record", str(record),
                     "-i", str(initial),"--pop_size", str(pop_size), "-k", str(k), "-R", str(R), "--simulations", str(simulations), "-T", str(T), '--decay_rate', str(decay_rate)])

              
        data_nm = np.load(out1 + "-nm.npz", allow_pickle="True")
        traj_nm.append([list(i) for i in data_nm['traj_record']][0])
        pop_size_nm.append(data_nm['pop_size'][0])
        ...
        inference.main(["--data", out1+"-nm.npz","--g1",str(g1),"-o",out2+'-nm',"-R", str(2), "--record", str(record), "--freq_cutoff", str(freq_cutoff), "--pop_size", str(int(pop_limit)), '-k', str(k)])
    ...
    np.savez_compressed(f, traj=traj, inferred=inferred, errors=errors, inferred_nm=inferred_nm, errors_nm=errors_nm, traj_nm=traj_nm,
                        actual=actual, mutant_sites=mutant_sites, pop_size=pop_size_normal, pop_size_nm=pop_size_nm)
    f.close
