...
def s_compare_scatter(file1, file2, ax=None, label1=None, label2=None, out=None):
    ...
    ax.plot(xy_line, xy_line, linewidth=SIZELINE, markersize=0, scalex=False, scaley=False)
    ...
    for line in ['bottom', 'left']:
        ax.spines[line].set_linewidth(SPINE_LW)
    ax.scatter(s1 * 100, s2 * 100, marker='.', s=0.15, color='k', alpha=1, edgecolors=None)
...
def plot_variant_traj(file=None, variants=[], var_sites=[], add_vars=[], add_sites=[], out='variant-comparison', min_freq=0.01, min_length=0):
    ...
    fig = plt.figure(figsize=[DOUBLE_COLUMN_WIDTH, DOUBLE_COLUMN_WIDTH * 1.4])
    ...
    if len(xticks)>5:
        ...
        xlabels = xlabels[::int(len(xticks) / 5)]
    ...
    plt.savefig(os.path.join(FIG_DIR, out + '.pdf'), dpi=1200)
