...
def main(args):
    ...
    sample_size   = arg_list.bsSample
    nruns         = arg_list.nruns
    ...
    #if sample_size < min_seqs:
    #    min_seqs = sample_size * delta_t    # If the sample size is small, only use the first and last day to calculate the frequency change delta x.
    ...
    status_name = f'covar-bs-{location}.csv'
    ...
    for run in range(nruns):
        print2(run)
        if os.path.exists(os.path.join(out_str, str(run), location + '.npz')):
            continue
        # subsample sequence data
        if nruns!=1:
            try: 
                os.mkdir(os.path.join(out_str, str(run)))
            except OSError as error:
                print(error)
            
        df_temp = sample_sequences(df, sample_size)
        ...
        if 'england' in location:
            counts = freqs_from_counts(counts, total_seqs, window=5, min_seqs=min_seqs, hard_window=True)
        else:
        ...
        if nruns==1:
            ...
            g = open(os.path.join(out_str, str(run), location + '.npz'), mode='wb')
        np.savez_compressed(
            g, 
            location=location, 
        ...
        if timed > 0:
            new_run_time = timer()
            print2(f'{new_run_time - bootstrap_start_time} seconds to run the {run} bootstrap calculation')
            bootstrap_start_time = new_run_time
