...
% update 08-Mar-2021
%         For the SIr work..includes the nR_t prefactor
% update 06-May-2020
...
% includes a betaN term mutiplied

function analysisStep1_v2_SIR_betaN_qI_new(fileNameContainingDirPath, priorConst, FLAG_vector, varargin)
...
tempDir = [dirNameData 'perSiteSelection' chosenSlash ];
textCell2{1} = 'Set';
tempReadFiles = findFileNamesWithGivenText(tempDir, textCell2);
...
dirNamePopN = [dirNameData 'perSiteSelection' chosenSlash];
%textCell1{1} = 'Set';

%fileNamePopN = findFileNamesWithGivenText(dirNamePopN, textCell1);
popN = dlmread([dirNamePopN 'populationN.txt']);
betaN = dlmread([dirNamePopN 'betaN.txt']);
q_Z_All = dlmread([dirNamePopN 'q_I.txt']);
if(runAnalysisCode)
        ...
        for bsamp = 1:numbSampleThisTP
            ...
            if(FLAG_Epi == true)
                ...
                if(FLAG_troubleShoot == true)
                    if(tp == 1)
                        b_vec_allTP = -1*ones(numUniqueTimePoints, numSitesNTExteded*(numSitesNTExteded-1)/2);
                    end
%                     numSitesNTExteded
%                     numSitesNTExteded*(numSitesNTExteded-1)/2
%                     size(b_vec)
%                     size(b_vec_allTP)
                    b_vec_allTP(tp,:) = b_vec;
                end
        ...
        if(FLAG_stratonovich == false)
            if(FLAG_linearInt == false)
                if(tp == 1)
                    ...
                    deltaX_vec = 0;
                    deltaZ_vec = 0;
                    temp_deltaX_vec_last = q_sp';
                    %temp_deltaZ_vec_last = q_z_sp';
        ...
        %                 pause
                ...
                elseif(tp == numUniqueTimePoints)
                    % MPL
                    q_T = q_sp';
                    temp_deltaX_vec = (q_sp' - temp_deltaX_vec_last);
                    deltaX_vec = deltaX_vec + temp_deltaX_vec*popN(tp-1)/betaN(tp-1);
                    
                    temp_deltaZ_vec = (q_z_sp' - temp_deltaX_vec_last);
                    deltaZ_vec = deltaZ_vec + temp_deltaZ_vec*popN(tp-1)/betaN(tp-1);
                    % MPL-Epi
                    if(FLAG_Epi == true)
                        qBar_T = [q_sp';b_sp'];
                    end
                    ...
                    deltaX_vec = deltaX_vec + temp_deltaX_vec*popN(tp-1)/betaN(tp-1);
                    deltaZ_vec = deltaZ_vec + temp_deltaZ_vec*popN(tp-1)/betaN(tp-1);
                    intCovMtx = intCovMtx +  tempintCovMtx*timeStep(tp)*popN(tp-1);
                    vEst1_noTS = sparse(1 - q_sp');
                    vEst2_noTS = - q_sp';
    ...
    fileNameSelEst_Z = ['SelEst_Z_' thisFileNameHUFiles(1:indOfDash(2)-1) '_' convention '_gamma' num2str(priorConstSC) '.txt'];
    ...
    numer = deltaX_vec;%(q_T - q_0 - vEstMu);
    numer_Z = deltaZ_vec;%
...
%        end
    ...
    if(FLAG_SaveFile)
        ...
        if(exist([dirNameAnalysis 'Estimates' chosenSlash fileNameSelEst_Z], 'file') == 2)
           delete([dirNameAnalysis 'Estimates' chosenSlash fileNameSelEst_Z]) 
        end
        ...
        dlmwrite([dirNameAnalysis 'Estimates' chosenSlash fileNameSelEst_Z], full(selEst_Z))
