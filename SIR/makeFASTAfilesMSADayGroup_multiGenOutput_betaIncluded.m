...
%%
function q = makeFASTAfilesMSADayGroup_multiGenOutput_betaIncluded(SetStrCell, numItr, numSampledSeqsPerStrainIn, genotypes, ngSamplingSchemeStr, samplingTime, perSiteSelection, dayGroup, TUsedForInfVec,exp_factor_Interp, betaTimeVarying_Interp_Mul_N)
...
for i = 1:size(genotypes, 2)
    ...
    set(gca, ...
     ...
     ylabel('Frequency')
     title(['Day group, Site: ' num2str(i)])
     if(i <= 8)
        set(gca, ...
          'XTickLabel', ' ')
     else
