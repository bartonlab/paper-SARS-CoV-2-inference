%%
% update: 13-Nov-2020, FLAG vector, FLAG to turn off synNonSyn computation
...
function preprocess_Step2_3_4(fileNameContainingDirPath, fileNameContainingMutProb, numNT, thisGenomicSegStartInd, thisGenomicSegStopInd, FLAGS_preProc_Steps_2_3_4)

if(length(FLAGS_preProc_Steps_2_3_4) == 3)
    FLAG_binaryApprox = FLAGS_preProc_Steps_2_3_4(1);
    FLAG_SaveFile = FLAGS_preProc_Steps_2_3_4(2);
    FLAG_Epi = FLAGS_preProc_Steps_2_3_4(3);
    FLAG_AssignSynNonSyn = true;
elseif(length(FLAGS_preProc_Steps_2_3_4) == 4)
    FLAG_binaryApprox = FLAGS_preProc_Steps_2_3_4(1);
    FLAG_SaveFile = FLAGS_preProc_Steps_2_3_4(2);
    FLAG_Epi = FLAGS_preProc_Steps_2_3_4(3);
    FLAG_AssignSynNonSyn = FLAGS_preProc_Steps_2_3_4(4);
end
...
close all
    ...
    %         end
        ...
        numNTsAtSite = sum(freqCountNTAllbSampAllTP ~= 0);
    end
    ...
    if(FLAG_AssignSynNonSyn == true)
        findIndexingAndMakeCSVFile(dirNameAnalysis, thisFileNameHUFiles, chosenSlash, refSeq, thisGenomicSegStopInd, thisGenomicSegStartInd, freqCountNTAllbSampAllTP, numFileNameHUFilesCell, consensusSeqTP1, numNT, synNonSyn_BinaryApprox);
         ...
         findIndexingAndMakeCSVFile(dirNameAnalysis, thisFileNameHUFiles, chosenSlash, refSeq, thisGenomicSegStopInd, thisGenomicSegStartInd, freqCountNTAllbSampAllTP, numFileNameHUFilesCell, consensusSeqTP1, numNT);
    end
