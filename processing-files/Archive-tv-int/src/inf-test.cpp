...
#include "inf-test.h"    // inference declarations
#include "io-test.h"     // input/output
...
void processStandard(const IntVVector &sequences, // vector of sequence vectors
    ...
    double *covtemp = new double[L*L];              // the integrated covariance matrix
    printf("%d\n",T);
    
    // set initial allele frequency and covariance then loop
    for (int a=0;a<L*L;a++) covtemp[a] = 0;
	...
	updateCovarianceIntegrate(times[k] - times[k-1], lastp1, lastp2, p1, p2, covtemp);
        //for (int i=0;i<L*L;i++) {totalCov[(k * L * L) + i] += covtemp[i]; }
	if (FILE *dataout = fopen(covarOutfile.c_str(),"a")) { printCovariance(dataout, covtemp, L); fclose(dataout);}
	//printf("%u\n",k);
...
void regularizeCovariance(const IntVVector &sequences, // vector of sequence vectors
                          ...
                          std::vector<double> &totalCov // integrated covariance matrix
                          ) {
...
int run(RunParameters &r) {
    ...
    std::string covarOutfile = r.getCovarianceOutfile();
    
    for (int a=0;a<  L;a++) dx[a]       = 0;
    //for (int a=0;a<T*L*L;a++) totalCov[a] = 0;
    
    // _ START TIMER
    auto t_start = Clock::now();
    
    if (r.useAsymptotic) processAsymptotic(sequences, counts, muMatrix, r.q, totalCov, dx);
    else                 processStandard(sequences, counts, times, muMatrix, r.q, dx, covarOutfile);
