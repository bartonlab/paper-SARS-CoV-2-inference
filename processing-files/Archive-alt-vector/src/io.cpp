...
//void printLinked(FILE* output, double linked[]) {
    
//    for (int i=0;i<linked.get_size();i++){
    
//        for (int j=0;j<linked[i].get_size();j++) fprintf(output,linked[i][j]);
//        fprintf(output,"\n");
...
void printDoubleCounts(FILE* output, const Vector &doublecounts, int L) {

    for (int i=0;i<L;i++) {

	fprintf(output,"%.4e ",doublecounts[i][0]);
	for (int j=1;j<L;j++) fprintf(output," %.4e ",doublecounts[i][j]);
	fprintf(output,"\n");
