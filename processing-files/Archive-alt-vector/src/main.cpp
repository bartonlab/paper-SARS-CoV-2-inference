...
/*********************************************************************
 ...
 -w: integer
    Default: 15
    Size of window (number of days) to use in smoothing the 
    frequencies when calculating the integrated covariance
    matrix and the frequency trajectories. Smoothing is 
    Gaussian by default.
...
int main(int argc, char *argv[]) {
    ...
    for (int i=1;i<argc;i++) {
        ...
        else if (strcmp(argv[i],"-w")==0)  { if (++i==argc) break; else r.w     = strtoint(argv[i]);                }
        ...
        else if (strcmp(argv[i],"-l")==0)  { if (++i==argc) break; else { r.linkOutfile   = argv[i]; r.saveLinked       = true; } }
	else if (strcmp(argv[i],"-dc")==0) { if (++i==argc) break; else { r.doubleOutfile = argv[i]; r.saveDoubleCounts = true; } }
